...
from nestforge.ir.emit_libnode import UnsupportedLibraryNode, data_edge, index_str
...
def test_clamped_index_exprs_are_emittable_and_exact():
    # a stencil/boundary-clamped subset: A[Max(0, i-1)] / bound Min(N, i+1); symstr renders `Max(0, i - 1)`,
    # which is no Python name
...
def test_unstructured_join_is_refused_not_double_applied():
    # emitting `k = k + 1` once per in-edge would double-increment a symbol only one predecessor assigns
...
def test_unspellable_array_dtype_is_refused_not_keyerror():
    # a complex/fp16/unsigned array must be a refusal naming the array and dtype, so the caller can keep the
    # DaceReference variant
...
def test_the_operator_agrees_with_the_helper_on_both_signs(a, b):
    ...
    assert eval(rendered) == int_floor(a, b)
...
def test_loop_init_statement_without_assignment_is_refused_not_indexerror():
    ...
    ``=`` (initialization happens elsewhere) must be a named refusal, not an ``IndexError``."""
