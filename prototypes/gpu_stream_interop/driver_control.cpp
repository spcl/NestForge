...
#define CK(x) do { cudaError_t e=(x); if(e){fprintf(stderr,"CUDA %s\n",cudaGetErrorString(e));return 2;} } while(0)

int main() {
    const long n = 1L << 22;                 // bigger => wider race window
    const size_t bytes = n * sizeof(double);
    double* h_a = (double*)malloc(bytes); double* h_out = (double*)malloc(bytes);
    ...
    CK(cudaMalloc(&d_a, bytes)); CK(cudaMalloc(&d_mid, bytes)); CK(cudaMalloc(&d_out, bytes));
    CK(cudaMemcpy(d_a, h_a, bytes, cudaMemcpyHostToDevice));

    long total_bad_runs = 0; const int ITERS = 30;
    for (int it = 0; it < ITERS; ++it) {
        ...
        long bad = 0;
        for (long i = 0; i < n; ++i) if (fabs(h_out[i] - (h_a[i]*2.0 + 1.0)) > 1e-12) ++bad;
        if (bad) ++total_bad_runs;
    }
    ...
    cudaFree(d_a); cudaFree(d_mid); cudaFree(d_out);
    free(h_a); free(h_out);
    return 0;
}
